...
- **D-12:** L'etiqueta d'una peça/forat és el seu **tag+class real** (`antena`, `orella`, `ull`, `contenidor-ulls`, `nas`, `boca`), que ja és descriptiu → serveix de etiqueta per si mateix (unifica "nom amable" i "tag": una decisió menys). **Només lectura**, mai editable (respecta el nucli anti-sintaxi del projecte). El detall visual exacte (chip net sense puntuació `< >` vs. tira de codi read-only que "s'escriu sola") queda a discreció fina d'UI/planner; totes dues respecten "real, descriptiu, mai editable".
  - **Override D-12 (2026-07-03, checkpoint Pla 02-03):** l'usuari demana explícitament mostrar el **tag HTML literal amb angle brackets** — `<img class="antena">`, `<span class="ull">`, `<button id="nas">`, `<output id="boca">`, i els contenidors `<section id="robot-contenidor">`, `<div id="robot-cap">`, `<div class="contenidor-ulls">` — "tal i com et vaig passar el esquema html" (D-01). Això **reverteix** la resolució de "sense angle brackets / nom pelat" que havia pres l'UI-SPEC (línia 75). Es manté el nucli anti-sintaxi: l'etiqueta segueix sent **text read-only** (`textContent`, mai `innerHTML`, cap camp editable, GAME-06/V5) — els `< >` són caràcters de text, no markup interpretat. Les etiquetes es deriven de `SLOTS[].html` / `CONTAINERS` (`pieceLabel`/`containerLabel` a `src/shared/robotTemplate.js`), única font de veritat, mai escrites a mà.
